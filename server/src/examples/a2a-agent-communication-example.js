...
import { A2AClient, A2ACardResolver } from "@a2a-js/sdk";
...
async function main() {
  ...
  try {
    ...
    if (!tradingAgent || !paymentAgent) {
      console.error("❌ Error: Could not find TradingAgent and PaymentProcessor");
    ...
    console.log(`      Skills: ${agentCard.skills?.map(s => s.name).join(", ") || "None"}\n`);

    // Step 3: Create A2A Client
    console.log("🔧 Step 3: Creating A2A Client...");
    
    // Initialize client with agent's base URL
    // A2AClient will fetch the agent card automatically
    ...
    console.log("💬 Step 4: TradingAgent sending message to PaymentProcessor...");
    const message = "Hello! I'm TradingAgent. Can you help me process a payment of 1 HBAR for a trading service?";
    console.log(`   Message: "${message}"\n`);

    const { MessageSendParams, SendMessageRequest } = await import("@a2a-js/sdk");
    const { v4: uuidv4 } = await import("uuid");

    const sendParams = new MessageSendParams({
      message: {
        role: "user",
    ...
    console.log("✨ Example complete! TradingAgent successfully communicated with PaymentProcessor using A2A protocol.\n");

...
本教程将指导你使用 Docker 快速部署 LIA AI 交易竞赛系统。
...
```bash
# 检查健康状态
docker inspect lia-backend | jq '.[0].State.Health'
docker inspect lia-frontend | jq '.[0].State.Health'
...
# 部署堆栈
docker stack deploy -c docker-compose.yml lia

# 查看服务状态
docker stack services lia

# 扩展服务
docker service scale lia_backend=3
```

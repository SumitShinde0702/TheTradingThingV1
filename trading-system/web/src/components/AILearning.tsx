...
export default function AILearning({ traderId }: AILearningProps) {
  ...
  if (error) {
    console.error('AILearning API error:', error);
    return (
      <div className="rounded p-6" style={{ background: '#1E2329', border: '1px solid #2B3139' }}>
        ...
        <div style={{ color: '#F6465D' }}>
          {t('loadingError', language)}: {error instanceof Error ? error.message : 'Unknown error'}
        </div>
  ...
  if (!performance || performance.total_trades === 0 || !performance.total_trades) {
    return (
      <div className="space-y-6">
        <div className="rounded p-6" style={{ background: '#1E2329', border: '1px solid #2B3139' }}>
          ...
          <div className="text-xs mt-2" style={{ color: '#848E9C', opacity: 0.7 }}>
            Waiting for completed trading cycles (open → close) to generate analysis data...
          </div>
        </div>

        {/* Always show footer with AI learning explanation */}
              ...
              {performance.sharpe_ratio !== undefined && (
                <div className="text-right mb-2">
                  <div className="text-sm font-bold px-3 py-1 rounded-lg" style={{
                    ...
                    {performance.sharpe_ratio >= 2 ? t('excellentPerformance', language) :
                     performance.sharpe_ratio >= 1 ? t('goodPerformance', language) :
                     performance.sharpe_ratio >= 0 ? t('highVolatility', language) : t('needsAdjustment', language)}
                  </div>
                </div>
              )}
            </div>
            ...
            {performance.sharpe_ratio !== undefined && (
              <div className="rounded-xl p-4" style={{
                ...
                <div className="text-sm leading-relaxed" style={{ color: '#DDD6FE' }}>
                  {performance.sharpe_ratio >= 2 && t('sharpeRatioExcellent', language)}
                  {performance.sharpe_ratio >= 1 && performance.sharpe_ratio < 2 && t('sharpeRatioGood', language)}
                  {performance.sharpe_ratio >= 0 && performance.sharpe_ratio < 1 && t('sharpeRatioFair', language)}
                  {performance.sharpe_ratio < 0 && t('sharpeRatioPoor', language)}
                </div>
              </div>
            )}
          </div>
          ...
          <div className="relative">
            ...
            <div className="rounded-xl p-4" style={{
              ...
              <div className="text-sm leading-relaxed" style={{ color: '#FEF3C7' }}>
                {(performance.profit_factor || 0) >= 2.0 && t('profitFactorExcellent', language, { value: (performance.profit_factor || 0).toFixed(1) })}
                {(performance.profit_factor || 0) >= 1.5 && (performance.profit_factor || 0) < 2.0 && t('profitFactorGood', language)}
                {(performance.profit_factor || 0) >= 1.0 && (performance.profit_factor || 0) < 1.5 && t('profitFactorFair', language)}
                {(performance.profit_factor || 0) > 0 && (performance.profit_factor || 0) < 1.0 && t('profitFactorPoor', language)}
              </div>
            </div>
          </div>
        </div>
...
function formatDuration(duration: string | undefined, language: 'en' | 'zh' | 'ko'): string {
  if (!duration) return '-';
  ...
  if (language === 'zh') {
    ...
    let result = '';
    if (hours) result += hours.replace('h', '시간');
    if (minutes) result += minutes.replace('m', '분');
    if (!hours && seconds) result += seconds.replace(/(\d+)\.?\d*s/, '$1초');
    return result || duration;
  } else {
    // English: keep original format or format nicely
    let parts: string[] = [];
    if (hours) parts.push(hours);
    if (minutes) parts.push(minutes);
    if (!hours && seconds) parts.push(seconds);

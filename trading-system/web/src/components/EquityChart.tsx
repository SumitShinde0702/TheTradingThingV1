...
export function EquityChart({ traderId }: EquityChartProps) {
  ...
  const { data: history, error } = useSWR<EquityPoint[]>(
    ...
    {
      refreshInterval: 3000, // 3秒刷新 - 实时更新图表
  ...
  const chartData = displayHistory.map((point) => {
    ...
    // Parse timestamp as UTC if no timezone info, then convert to Singapore time
    const ts = point.timestamp;
    let parsedTime: Date;
    if (ts.includes('T') || ts.includes('Z')) {
      parsedTime = new Date(ts);
    } else {
      // Treat database timestamps without timezone as UTC

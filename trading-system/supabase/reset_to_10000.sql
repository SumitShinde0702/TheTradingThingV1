-- Reset Database to 10000 USDT Starting Balance (Complete Reset)
-- Run this script in Supabase SQL Editor to reset all traders back to 10000 USDT
-- 
-- This will:
-- 1. Delete all existing records (decisions, positions, decision_actions)
-- 2. Reset ID sequences to start from 1 (fresh ID numbering)
-- 3. Seed cycle #0 with 10000 USDT for both traders
-- 4. System will restart from cycle #1 after this

BEGIN;

-- Step 1: Delete all existing records (in correct order due to foreign keys)
DELETE FROM decision_actions;
DELETE FROM positions;
DELETE FROM decisions;

-- Step 1.5: Reset ID sequences to start from 1 (fresh start)
ALTER SEQUENCE decisions_id_seq RESTART WITH 1;
ALTER SEQUENCE positions_id_seq RESTART WITH 1;
ALTER SEQUENCE decision_actions_id_seq RESTART WITH 1;

-- Step 2: Seed initial balance records (cycle #0) with 10000 USDT for both traders
INSERT INTO decisions (
    trader_id, 
    timestamp, 
    cycle_number, 
    input_prompt, 
    cot_trace, 
    decision_json, 
    raw_response,
    success, 
    error_message,
    account_total_balance, 
    account_available_balance, 
    account_unrealized_profit,
    account_position_count, 
    account_margin_used_pct,
    execution_log, 
    candidate_coins
) VALUES
    -- OpenAI Trader
    (
        'openai_trader',
        CURRENT_TIMESTAMP,
        0,
        'Initial seed record',
        '',
        '{"seed": true}',
        '',
        true,
        NULL,
        10000.0,
        10000.0,
        0.0,
        0,
        0.0,
        '[]',
        '[]'
    ),
    -- Qwen Trader
    (
        'qwen_trader',
        CURRENT_TIMESTAMP,
        0,
        'Initial seed record',
        '',
        '{"seed": true}',
        '',
        true,
        NULL,
        10000.0,
        10000.0,
        0.0,
        0,
        0.0,
        '[]',
        '[]'
    )
ON CONFLICT (trader_id, cycle_number) DO UPDATE SET
    account_total_balance = 10000.0,
    account_available_balance = 10000.0,
    account_unrealized_profit = 0.0,
    account_position_count = 0,
    account_margin_used_pct = 0.0,
    timestamp = CURRENT_TIMESTAMP;

-- Verify the reset
DO $$
DECLARE
    openai_count INTEGER;
    qwen_count INTEGER;
    openai_balance REAL;
    qwen_balance REAL;
BEGIN
    -- Check OpenAI trader
    SELECT COUNT(*) INTO openai_count
    FROM decisions 
    WHERE trader_id = 'openai_trader' AND cycle_number = 0;
    
    SELECT account_total_balance INTO openai_balance
    FROM decisions 
    WHERE trader_id = 'openai_trader' AND cycle_number = 0
    LIMIT 1;
    
    -- Check Qwen trader
    SELECT COUNT(*) INTO qwen_count
    FROM decisions 
    WHERE trader_id = 'qwen_trader' AND cycle_number = 0;
    
    SELECT account_total_balance INTO qwen_balance
    FROM decisions 
    WHERE trader_id = 'qwen_trader' AND cycle_number = 0
    LIMIT 1;
    
    RAISE NOTICE 'Reset Complete!';
    RAISE NOTICE 'openai_trader: % records, Balance: % USDT', openai_count, openai_balance;
    RAISE NOTICE 'qwen_trader: % records, Balance: % USDT', qwen_count, qwen_balance;
END $$;

COMMIT;

-- After running this script:
-- ⚠️  IMPORTANT: You MUST restart the backend server after running this script!
-- 
-- Why? The paper trader stores positions in-memory. Even though the database is reset,
-- the running backend still has old positions in memory. Restart is required to:
-- 1. Clear in-memory positions
-- 2. Restore state from cycle #0 (10000 USDT, no positions)
-- 3. Start fresh from cycle #1
--
-- Steps:
-- 1. Run this SQL script in Supabase SQL Editor
-- 2. Restart the backend on Render (or redeploy)
-- 3. The system will restore from cycle #0 and start with 10000 USDT
-- 4. New cycles will start from #1 with no positions

